...
export async function upsertFromLaunch(claims: LaunchClaims): Promise<void> {
  ...
  await sql`
    insert into coingame_instance (room_id, host_origin, return_url)
    values (${claims.roomId}, ${origin}, ${claims.returnUrl})
    on conflict (room_id) do update set return_url = excluded.return_url`;
  await sql`
    insert into coingame_player (player_id, display_name, avatar_url, last_seen_at)
    values (${claims.playerId}, ${claims.displayName}, ${claims.avatar ?? null}, now())
    on conflict (player_id) do update
      set display_name = excluded.display_name,
          avatar_url = coalesce(excluded.avatar_url, coingame_player.avatar_url),
          last_seen_at = now()`;
}

export async function hostOriginFor(roomId: string): Promise<string | null> {
  const rows = await sql`select host_origin from coingame_instance where room_id = ${roomId}`;
